...
public struct CleanupCommandFactory {
        ...
        case .git:
            cmds.append(contentsOf: [
                ...
                CleanupCommand(
                    ...
                    riskLevel: .danger,
                    command: "rm -rf \(ShellEscaper.quote(entry.path))",
    ...
    public func optimizeCommand(for entry: TargetEntry) -> CleanupCommand? {
        ...
        case .next:
            return CleanupCommand(
                ...
                riskLevel: .safe,
                command: "rm -rf \(ShellEscaper.quote(entry.path))/cache",
                requiresWarning: false
            )
        case .terraform:
            return CleanupCommand(
    ...
    public func hardDeleteCommand(roots: [String], kind: TargetKind) -> String {
        let quoted = roots.map { ShellEscaper.quote($0) }.joined(separator: " ")
        return "find \(quoted) -type d -name \(ShellEscaper.quote(kind.folderName)) -prune -exec rm -rf {} +"
    }
