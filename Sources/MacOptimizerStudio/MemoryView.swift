...
struct MemoryView: View {
    ...
    @AppStorage("has_seen_resource_coach_mark") private var hasSeenResourceCoachMark = false
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: DesignTokens.sectionSpacing) {
                ...
                if !hasSeenResourceCoachMark {
                    HStack(spacing: 8) {
                        Image(systemName: "info.circle.fill")
                            .foregroundStyle(.blue)
                        Text("MacOptimizer Studio uses a small amount of CPU and memory for real-time monitoring. This is normal and expected — the app hides itself from the process list to avoid confusion.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Spacer(minLength: 0)
                        Button {
                            hasSeenResourceCoachMark = true
                        } label: {
    ...
    private func filteredProcesses(_ processes: [ProcessMemoryEntry]) -> [ProcessMemoryEntry] {
        let filtered = processes.filter { !$0.name.contains("MacOptimizerStudio") }
        if processFilter.isEmpty { return filtered }
        let needle = processFilter.lowercased()
        return filtered.filter { $0.name.lowercased().contains(needle) }
    }
