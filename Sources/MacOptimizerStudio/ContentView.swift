    ...
    private var activeSectionView: some View {
        ...
        case .home:
            HomeView(
                ...
                openDocker: { selectedSection = .docker },
        ...
        case .maintenance:
            MaintenanceView().sectionTransition()
        case .storageTools:
            StorageToolsView().sectionTransition()
        case .loginItems:
        ...
        case .photoJunk:
            PhotoJunkView().sectionTransition()
        case .shredder:
            FileShredderView().sectionTransition()
        case .updater:
            UpdaterView().sectionTransition()
        case .extensions:
            ExtensionManagerView().sectionTransition()
        case .network:
            NetworkView().sectionTransition()
        case .diskHealth:
            DiskHealthView().sectionTransition()
        case .startupTime:
            StartupTimeView().sectionTransition()
        case .diskBenchmark:
            DiskBenchmarkView().sectionTransition()
        case .brokenDownloads:
            BrokenDownloadsView().sectionTransition()
        case .screenshotOrganizer:
            ScreenshotOrganizerView().sectionTransition()
        case .activityLog:
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            if showBottomHint {
                VStack(spacing: 0) {
                    LinearGradient(
                        colors: [Color(nsColor: .windowBackgroundColor).opacity(0), Color(nsColor: .windowBackgroundColor).opacity(0.95)],
                        startPoint: .top,
                        endPoint: .bottom
                    )
                    .frame(height: 24)
    ...
    private var cleanupStep: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                featureCard(
                    icon: "shippingbox", color: .purple,
                    title: "Docker",
                    detail: "View images, volumes, and containers with sizes. Remove individual items or prune unused resources in bulk.",
                    badge: nil
                )
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 160), spacing: 10)], spacing: 10) {
                    miniFeature(icon: "wrench.and.screwdriver", title: "Maintenance", detail: "System scripts")
                    miniFeature(icon: "chart.bar.doc.horizontal", title: "Storage Tools", detail: "Large file finder")
                    miniFeature(icon: "photo.on.rectangle.angled", title: "Photo Junk", detail: "Screenshot cleanup")
                    miniFeature(icon: "arrow.down.circle.dotted", title: "Downloads", detail: "Broken files")
                    miniFeature(icon: "photo.on.rectangle.angled", title: "Screenshots", detail: "Organize by date")
                    miniFeature(icon: "square.stack.3d.up", title: "Apps", detail: "Full uninstaller")
                }
            }
            .padding(24)
        }
