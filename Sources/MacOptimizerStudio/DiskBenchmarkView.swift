    ...
    private func resultsCard(_ result: BenchmarkResult) -> some View {
        StyledCard {
            VStack(alignment: .leading, spacing: 10) {
                CardSectionHeader(icon: "chart.bar.doc.horizontal", title: "Detailed Results", color: .blue)

                Divider()

                LazyVGrid(columns: [GridItem(.adaptive(minimum: 200), spacing: 12)], spacing: 10) {
                    InfoRow(label: "Sequential Read", value: String(format: "%.1f MB/s", result.sequentialReadMBps))
                    InfoRow(label: "Sequential Write", value: String(format: "%.1f MB/s", result.sequentialWriteMBps))
                    if let iops = result.randomReadIOPS {
                        InfoRow(label: "Random Read IOPS", value: formatValue(iops))
                    }
                    if let iops = result.randomWriteIOPS {
                        InfoRow(label: "Random Write IOPS", value: formatValue(iops))
                    }
                    InfoRow(label: "Test File Size", value: "\(result.fileSizeMB) MB")
                    InfoRow(label: "Test Path", value: result.testPath)
                }
